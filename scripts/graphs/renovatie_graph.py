...
def create_renovatie_graph():
    # Zorg ervoor dat de paden correct zijn ten opzichte van het huidige script
    base_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..'))
    input_file = os.path.join(base_dir, 'data', 'renovatie_pivot.csv')
    output_file = os.path.join(base_dir, 'graphs', 'renovatie_pivot.html')
    ...
    fig = px.line(
        df,
        x='Year-Quarter',
        y=['eengezinswoning', 'meergezinswoning'],
        labels={
            'value': 'Aantal wooneenheden',
            'variable': 'Type woning',
            'Year-Quarter': 'Kwartaal'
        },
        title='Vergunningsaanvragen voor renovatie'
    )
